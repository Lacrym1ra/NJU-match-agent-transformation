...
export async function down(db: PostgresJsDatabase<any>) {
  await db.execute(`
    DROP INDEX IF EXISTS idx_circle_chat_read_states_user_updated;
    DROP INDEX IF EXISTS idx_circle_chat_read_states_user_circle;
    DROP TABLE IF EXISTS circle_chat_read_states CASCADE;

    DROP INDEX IF EXISTS idx_circle_chat_messages_sender_client;
    DROP INDEX IF EXISTS idx_circle_chat_messages_sender_created;
    DROP INDEX IF EXISTS idx_circle_chat_messages_circle_created;
    DROP TABLE IF EXISTS circle_chat_messages CASCADE;
  `);
}
